...
Server.prototype.onResponse = function(func) {
  this.responseCallbacks.push(func);
}

Server.prototype.respond = function(response) {
  console.log("response = ", response);
  var callback = this.responseCallbacks[0];
  if (callback.quiet && response.header.totalBodyLength > 0) {
    callback(response);
  } else {
    this.responseCallbacks.shift()(response);
    this.errorCallbacks.shift();
  }
}

Server.prototype.onError = function(func) {
  this.errorCallbacks.push(func);
}

Server.prototype.error = function(err) {
  this.errorCallbacks.shift()(err);
